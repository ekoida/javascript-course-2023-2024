...
const render = (parentElement, data) => {
  ...
  data.employees.forEach((employee) => { // запускаем arrow функцию, которая...
...
//Object.keys -метод, который возвращает названия свойств объекта(keys) и к каждому key применяет метод forEach ...
    Object.keys(employee).forEach((emploeeKey) => {//..который запускает arrow функцию..
      td = document.createElement("td");// по созданию элемнта "td" - переменная td
      if (emploeeKey === "photo") { //если свойсво объекта(emploeeKey) - это "photo" ...
      ...
      } else if (emploeeKey === "contacts") {//если свойсво объекта(emploeeKey) - это "contacts"...
        ...
        Object.keys(employee["contacts"]).forEach((contactsKey) => {
          if (employee["contacts"][contactsKey] !== "") {//если (описывается путь к содержанию? key не пустая строка)..
            ...
            if (contactsKey === "email") {//если свойсво объекта(emploeeKey) -это "email"...
              prefix = "mailto:";// тогда ....
      ...
      } else {//если это не контакты запиши значение оставшихся Key
